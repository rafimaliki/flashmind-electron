...
function IconBtn({ icon: Icon, label, danger, onClick, small }) {
  ...
  return (
    <button
      ...
      onMouseEnter={e => {
        e.currentTarget.style.background = danger ? 'rgba(248,113,113,0.12)' : 'var(--bg-overlay)'
...
function ProfileItem({ profile, isActive, onClick }) {
  return (
    ...
    >
      ...
      <p className="text-[11px] mt-0.5 font-mono" style={{ color: isActive ? 'rgba(245,158,11,0.6)' : 'var(--text-muted)' }}>
        {profile.folders.length} {profile.folders.length === 1 ? 'folder' : 'folders'}
      </p>
    </button>
  )
}
      ...
      <div className="flex-1 overflow-y-auto px-2 py-2 space-y-0.5">
        ...
        {!loading && profiles.length === 0 && (
          <div className="px-3 py-6 text-center">
            <p className="text-xs" style={{ color: 'var(--text-muted)' }}>No profiles yet.</p>
            <button onClick={onNew} className="text-xs mt-2 underline"
  ...
  return (
    <div className="h-full overflow-y-auto p-8 max-w-2xl mx-auto">
      ...
      <div className="mb-8 anim-item" style={{ animationDelay: '0.10s' }}>
        ...
        {profile.folders.length === 0 ? (
          ...
          >
            ...
            <p className="text-sm mb-1" style={{ color: 'var(--text-secondary)' }}>No folders added</p>
          ...
          <div className="space-y-2">
            {profile.folders.map(fp => (
              <FolderItem key={fp} folderPath={fp} onRemove={() => onRemoveFolder(fp)} />
            ))}
            <div className="pt-1">
              <AddFolderButton onClick={onAddFolder} />
            </div>
      ...
      <div className="anim-item" style={{ animationDelay: '0.20s' }}>
        ...
        <div className="rounded-xl p-5"
             style={{ background: 'var(--bg-elevated)', border: '1px solid rgba(248,113,113,0.15)' }}>
          {!showDeleteConfirm ? (
  ...
  return (
    <div className="h-full flex items-center justify-center">
      <div className="text-center anim-item">
        <FolderOpen size={40} style={{ color: 'var(--text-muted)', margin: '0 auto 16px' }} />
...
export default function Profiles({ onStartSession }) {
  ...
  async function handleUpdateCardsPerSession(value) {
    if (!selectedId) return
    const updated = await api.profiles.update(selectedId, { cardsPerSession: value })
    setProfiles(prev => prev.map(p => p.id === selectedId ? updated : p))
  }
